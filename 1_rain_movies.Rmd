...
Added error bars to the previous graph.

```{r}
p3 <- ggplot(daily_p,aes(x=date,y=mean_temp,color=station,group=station)) + 
  geom_line() + 
  geom_point() +
  geom_errorbar(
...
animate(p3, nframes=nrow(daily_p))


```

Actualy try to plot correlation based on color and two sites on axis  

```{r}
daily_long <- daily_p %>%
  select(date, station, mean_temp, min_temp, max_temp)%>%
  gather(.,
         key = "type",
         value = "temp")
daily_wide <- daily_temp_long %>%
  spread(., 
         key = "station",
         value = "temp")

daily_wide_order <- arrange(daily_wide, type)

p4 <- ggplot(daily_wide_order,
       aes(x=AFF, y=FNL, color = type,group = seq_along(FNL))) +
  geom_point() +
  #geom_line()+
  labs(x = 'Air-Force Temp', y= 'Fort-Collins Temp' )+
  transition_reveal(date) +
  theme(legend.title=element_blank())
  
gganimate::animate(p4, nframes = 75,
  width = 700, height = 700, res = 50)
```
...
mean_t_sf_sliced <- daily_p %>%
  slice(1:100) %>%
  st_as_sf(.,coords=c('lon','lat'),crs=4326,remove=F)

full_dates_sliced <- unique(daily_sf$date) %>%
  slice(1:30)

sequence <- 1:length(full_dates)
saveGIF({
  for(i in sequence){
  sub_sf_two <- mean_t_sf_sliced %>%
    filter(date == full_dates_sliced[i]) %>%
    filter(daily_p > 0)
  

  if(nrow(sub_sf) > 0){
  map <- basemap + tm_shape(sub_sf_two) + 
    tm_dots(title.size='Mean Temp in Celsius',
            col='mean_temp',
            size='mean_temp',
            style='cont',
            palette='Blues',
